...
class Emotion:
    ...
    def findAllEmotions(self):
        ...
        if len(data) == 0:
            ...
            result['message'] = "Terjadi kesalahan saat mengambil semua data emosi"

        if status == True and len(data) != 0:
            result['status'] = True
            result['data'] = data
            result['message'] = "Berhasil mengambil semua data emosi"
        return result
    ...
    def insertEmotion(self, data):
        ...
        if statusInsert == True and dataInsert != None:
            result['status'] = True
            result['data'] = {'_id': data['_id'], 'username': data['username'], 'date': data['date'], 'emotion': data['emotion']}
            result['message'] = "Berhasil insert data emosi"
        return result